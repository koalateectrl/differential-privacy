...
flags.DEFINE_integer('count', 50, 'Number of labeled examples per class')
...
def make_discriminator_models(input_shape = (28, 28, 1), n_classes = 10):
  ...
  flat = tf.keras.layers.Flatten()(img_inputs)
  gnl1 = tf.keras.layers.GaussianNoise(stddev = 0.3)(flat)
  dense1 = tf.keras.layers.Dense(1000)(gnl1)

  gnl2 = tf.keras.layers.GaussianNoise(stddev = 0.5)(dense1)
  dense2 = tf.keras.layers.Dense(500)(gnl2)

  gnl3 = tf.keras.layers.GaussianNoise(stddev = 0.5)(dense2)
  dense3 = tf.keras.layers.Dense(250)(gnl3)

  gnl4 = tf.keras.layers.GaussianNoise(stddev = 0.5)(dense3)
  dense4 = tf.keras.layers.Dense(250)(gnl4)

  gnl5 = tf.keras.layers.GaussianNoise(stddev = 0.5)(dense4)
  dense5 = tf.keras.layers.Dense(250)(gnl5)

  gnl6 = tf.keras.layers.GaussianNoise(stddev = 0.5)(dense5)
  dense6 = tf.keras.layers.Dense(n_classes)(gnl6)

  s_out_layer = tf.keras.layers.Activation('softmax')(dense6)
  s_model = tf.keras.Model(img_inputs, s_out_layer)

  d_out_layer = tf.keras.layers.Lambda(custom_activation)(dense6)
...
def create_data_set():
  ...
  num_labeled_examples = FLAGS.count * FLAGS.nb_labels
  X_sup = X_test[:num_labeled_examples]
  y_sup = y_test[:num_labeled_examples]

  X_unsup = X_test[num_labeled_examples:9000]
  ...
  supervised_ds = tf.data.Dataset.from_tensor_slices((X_sup, y_sup)).shuffle(num_labeled_examples).batch(FLAGS.batch_size)
  unsupervised_ds = tf.data.Dataset.from_tensor_slices(X_unsup).shuffle(9000 - num_labeled_examples).batch(FLAGS.batch_size)
...
def main(unused_argv):
  ...
  # Used for training unsupervised portion of model since half is real data and half is generated
